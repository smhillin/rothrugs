...
class ControllerProductSearch extends Controller {
    ...
    public function index() {
        ...
        if (isset($this->request->get['limit'])) {
            ...
            $this->session->data['search_limit'] = $limit;
        } else {
            if(isset($this->session->data['search_limit']) && (int)$this->session->data['search_limit'] > 0){
                $limit = $this->session->data['search_limit'];
            }else{
        ...
        foreach ($results as $result) {
            ...
            // if (empty($image)) {
                if (!empty($result['image']) && $result['image'] != 'catalog/default/default.png') {
            ...
            /*$roth_img = strstr($image, 'https://s3.amazonaws.com/rothrugsimg/RothRugs/');
            if($roth_img){
                $thumb_img = explode('RothRugs/',$image);
                $image = 'https://s3.amazonaws.com/rothrugsimgresized/RothRugs/'.$thumb_img[1];
            }*/
...
//        }
        ...
        foreach ($limits as $value) {
            if ($value == $this->config->get('config_product_limit')) {
                $data['limits'][] = array(
                    'text' => 'Show - '.$value .' products',
                    'value' => $value,
...
//        echo "aboveURL:-" . $url . "</br>";
        ...
        $data['results_mobile_bot'] = 'RESULTS  showing ' . $datapr2 . ' of ' . $product_total . '';
