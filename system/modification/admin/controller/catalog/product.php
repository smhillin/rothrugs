...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		foreach ($product_images as $product_image) {
			...
			if (is_file(DIR_IMAGE . $product_image['image'])) {
				$image = $product_image['image'];
				$thumb = $thumb = $this->model_tool_image->resize($product_image['image'], 100, 100);
			} else {
				// $image = '';
				// $thumb = 'no_image.png';
				if (!empty($product_image['image'])) {
                    $a = strstr($product_image['image'], 'https://s3.amazonaws.com/');
                    $aa = strstr($product_image['image'], 'https://cdn1-media.s3.amazonaws.com/');
                    if (!empty($a) || $aa) {
                        $thumb = $image = $product_image['image'];
                    } else {
                        $b = strstr($product_image['image'], 'http://www.owimages.com/');
                        if (!empty($b)) {
                            $thumb = $image = $product_image['image'];
                        } else {
                            $image = '';
