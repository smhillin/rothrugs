...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', landing_page = '" . $this->db->escape($data['landing_page']) . "'");
		}

		if(isset($data['combination'])){
			foreach ($data['combination'] as $key => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_to_combination SET category_id = '" . (int)$category_id . "', combination = '" . (string)$value . "'");
			}
		}
		...
		if (isset($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_id=" . (int)$category_id . "',  keyword = '" . $this->db->escape($data['keyword']) . "' , lang = '".(int)$this->config->get('config_language_id')."'");
		}
	...
	public function editCategory($category_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias  WHERE query = 'category_id=" . (int)$category_id . "' AND lang = '" . (int)$this->config->get('config_language_id') . "'");

		if ($data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_id=" . (int)$category_id . "',  keyword = '" . $this->db->escape($data['keyword']) . "' , lang = '".(int)$this->config->get('config_language_id')."'");
		}

		$this->db->query("DELETE FROM " . DB_PREFIX . "category_to_combination WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCombinations($category_id) {
		$answer_combinations = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_to_combination WHERE category_id = '" . (int)$category_id . "'");

		foreach ($query->rows as $result) {
			$answer_combinations[] = $result['combination'];
		}
