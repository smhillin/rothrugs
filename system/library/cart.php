...
class Cart {
		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$tt = ($price + $option_price) * $quantity;
					/*if($product_query->row['manufacturer_id'] == 12)
					{
						$product_total = $tt - ($tt / 100 * 35);
					}
					elseif($product_query->row['manufacturer_id'] == 15)
					{
						$product_total = $tt - ($tt / 100 * 30);
					}
					elseif($product_query->row['manufacturer_id'] == 13)
					{
						$product_total = $tt - ($tt / 100 * 15);
					}
					elseif($product_query->row['manufacturer_id'] == 11)
					{
						$product_total = $tt - ($tt / 100 * 20);
					}else{
						$product_total = $tt;
					}*/
					$product_total = $tt;
	...
	public function getSubTotal() {
		...
		foreach ($this->getProducts() as $product) {
			...
			{
				$product['total'] = $product['total'] - ($product['total'] / 100 * 35);
			}
			elseif($product['manufacturer_id'] == 15)
			{
				$product['total'] = $product['total'] - ($product['total'] / 100 * 30);
			}
			elseif($product['manufacturer_id'] == 13)
			{
				$product['total'] = $product['total'] - ($product['total'] / 100 * 15);
			}
			elseif($product['manufacturer_id'] == 11)
			{
				$product['total'] = $product['total'] - ($product['total'] / 100 * 20);
			}*/
