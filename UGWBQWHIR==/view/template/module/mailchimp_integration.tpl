...
<script type="text/javascript"><!--
	...
	<?php if (!empty($used_rows)) { ?>
		var usedRows = {
			<?php foreach ($used_rows as $table => $values) { ?>
				<?php echo $table; ?>: [<?php echo implode(',', $values); ?>],
			<?php } ?>
		}
	<?php } ?>
	...
	function copyRow(row) {
		...
		if (row.parents('table').attr('data-autoincrement') != undefined) {
			...
			while (usedRows[row.attr('class')].indexOf(firstUnusedRow) != -1) {
				firstUnusedRow++;
			}
			usedRows[row.attr('class')].push(firstUnusedRow);
		}
		clone.html(clone.html().replace(new RegExp(row.attr('class') + '_(\\d+)_', 'g'), row.attr('class') + '_' + firstUnusedRow + '_'));
