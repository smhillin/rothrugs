...
class ControllerUserUser extends Controller {
	...
	public function index() {
		$query = $this->db->query("DESC `" . DB_PREFIX . "user` `uksb_2fa_status`");
		if (!$query->num_rows) {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->request->post['uksb_2fa_secret'] = ($this->request->post['uksb_2fa_status'] ? $this->request->post['uksb_2fa_secret'] : '');
	...
	protected function getForm() {
		...
		$data['entry_2fa_title'] = $this->language->get('entry_2fa_title');
