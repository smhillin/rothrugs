...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $powertrack_trackcode = '', $powertrack_carrier = '') {
...
if ($this->config->get('mailchimp_integration_ecommerce360') && file_exists(DIR_SYSTEM . 'library/mailchimp_integration_pro.php')) {
			...
			if ($order_info['order_status_id'] && $order_status_id) {
				...
				$message .='<a href="http://rothrugs.com/" title="Rothrugs">';
				$message .= '<img src="http://rothrugs.com/catalog/view/theme/rr2/img/logo.png" alt="Rothrugs">'. "\n\n";
				$message .='</a>';
				...
				if ($order_status_query->num_rows) {
					$message .= $language->get('text_update_custom_1') . $order_status_query->row['name'];
				}
				if ($notify && $comment) {
					$message .= "<br><br>\n\n";
					$message .= (nl2br(strip_tags($comment))) . "\n\n";
				}
				...
				/*$message .='<p>';
				if ($order_info['customer_id']) {
					$message .= $language->get('text_update_custom_1') . "\n\n";
					$message .= '<a href="'.$order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id.'" title="">'.$order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id.'</a>' . "\n\n";
				}
				$message .='</p>';*/

				$message .='<p>';
				$message .= $language->get('text_update_custom_3') . "\n\n";
				$message .='</p>';
