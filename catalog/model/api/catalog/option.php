<?php
class ModelApiCatalogOption extends Model {
	...
	public function getOptionDescription($option_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_value_description` ORDER BY  `oc_option_value_description`.`option_value_id` DESC LIMIT 1 ");
		return $query->row;
	}
	public function getAllOptionsById($option_id = ''){
		$product_option_data = array();
		if(!empty($option_id))
			$options = $this->db->query("SELECT ov.*, oc.name AS option_name FROM " . DB_PREFIX . "option_value_description ov
													INNER JOIN " . DB_PREFIX . "option_description oc ON ov.option_id = oc.option_id WHERE oc.option_id = '" . $option_id . "'");
		else
			$options = $this->db->query("SELECT ov.*, oc.name AS option_name FROM " . DB_PREFIX . "option_value_description ov
													INNER JOIN " . DB_PREFIX . "option_description oc ON ov.option_id = oc.option_id");
		foreach($options->rows as $opt){
			$product_option_data[$opt['option_name']][] = $opt;
		}
