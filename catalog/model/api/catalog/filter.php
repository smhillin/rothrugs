<?php
class ModelApiCatalogFilter extends Model {
	...
	public function addFilter($data){
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "filter_group_description SET filter_group_id = '" . (int) $filter_group_id . "', language_id = '" . (int) $this->language_id . "', name = '" . $this->db->escape($data['filter_group_name']) . "'");
		...
		if(isset($data['filter'])){
			foreach($data['filter'] as $filter){
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int) $filter_id . "', language_id = '" . (int) $this->language_id . "', filter_group_id = '" . (int) $filter_group_id . "', name = '" . $this->db->escape($filter['filter_name']) . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		foreach ($data['filter_group_description'] as  $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "filter_group_description SET filter_group_id = '" . (int)$filter_group_id . "', language_id = '" . (int)$this->language_id . "', name = '" . $this->db->escape($value['name']) . "'");
		}
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				...
				foreach ($filter['filter_description'] as  $filter_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)  $this->language_id . "', filter_group_id = '" . (int)$filter_group_id . "', name = '" . $this->db->escape($filter_description['name']) . "'");
				}
			}
		}
	}
