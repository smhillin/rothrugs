...
class ModelCatalogReview extends Model {
	...
	public function getAllReviewsByProductId($product_id, $start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "review_product WHERE product_id = '" . (int)$product_id . "' AND status = '1' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
