				...
				foreach ($this->cart->getProducts() as $product) {
					...
					if ($status) {
						if ($coupon_info['type'] == 'F') {
							...
							if(isset($coupon_info['product_discount_data'][$product['product_id']]) && $coupon_info['product_discount_data'][$product['product_id']] > 0){
								$discount = $product['total'] / 100 * $coupon_info['product_discount_data'][$product['product_id']];
							}else{
