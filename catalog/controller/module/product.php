...
//			}
    ...
    protected function updateInsert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if(!empty($check) && $produc_insert['type'] == 'manufacturer')
            {
                $product_id = $check['product_id'];
                $produc_insert['product_store'] = array(0);
                $product_ids = $this->model_api_catalog_product->editProductManufacturer($product_id, $this->request->post);
...
// ZZZ
    ...
    /*protected function updatesize() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $product_detail = $this->request->post;
            
            $check = $this->model_api_catalog_product->getProductsBySku($product_detail['sku']);
            if ($check) {
                $product_id = $check['product_id'];
                $this->model_api_catalog_product->editProductSize($product_id, $this->request->post);
                $data = array('status' => true, 'data' => 'size_update');
                return $this->response->setOutput(json_encode($data));
            }
        }
        $this->getError();
    ...
    protected function updatesizeoption() {
        $this->load->language('api/product');
        $this->load->model('api/catalog/product');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $product_detail = $this->request->post;
            ...
            if ($check) {
                $product_id = $check['product_id'];
                $size_name = $product_detail['size'];
                if($this->model_api_catalog_product->editProductSizeOption($product_id, $size_name))
                {
                    $data = array('status' => true, 'data' => 'size_option_update');
                    return $this->response->setOutput(json_encode($data));
                }
            }
        }
    ...
    protected function updateSpecs() {
        $this->load->language('api/product');
        $this->load->model('api/catalog/product');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($check) {
                $product_id = $check['product_id'];
                $produc_insert['product_store'] = array(0);
                $product_ids = $this->model_api_catalog_product->editProductSpecs($product_id, $this->request->post);
                $data = array('status' => true, 'data' => 'updatespecs');
                return $this->response->setOutput(json_encode($data));
            }
        }
        $this->getError();
