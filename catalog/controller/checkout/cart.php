		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($products as $product) {
				...
				if (!empty($product['image'])) {
                    $a = strstr($product['image'], 'https://s3.amazonaws.com/');
                    $aa = strstr($product['image'], 'https://cdn1-media.s3.amazonaws.com/');
                    if (!empty($a) || $aa) {
                        $image = $product['image'];
                    } else {
                        $b = strstr($product['image'], 'http://www.owimages.com/');
                        if (!empty($b)) {
                            $image = $product['image'];
                        } else {
                            $image = $this->model_tool_image->resize($product['image'], $this->config->get('config_image_cart_width'), $this->config->get('config_image_cart_height'));
				...
				$data['products'][] = array(
					...
					'price'     => $price,
					'unit_discout_price' => $product['unit_discout_price'],
					'total'     => $total,
